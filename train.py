...
import argparse
import os
from src.src_anti.train_main import TrainMain
from src.src_anti.default_config import get_default_config, update_config
...
def parse_args():
    ...
    cuda_devices = [int(elem) for elem in t_args.device_ids]
    os.environ["CUDA_VISIBLE_DEVICES"] = ','.join(map(str, cuda_devices))
    t_args.devices = [x for x in range(len(cuda_devices))]
    return t_args
